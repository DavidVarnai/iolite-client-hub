  ...
  return clientAssignments.map(assignment => {
    ...
    for (const comp of memberComps) {
      switch (comp.componentType) {
        case 'salary_allocation': {
          ...
          totalCost += cost;
          parts.push(`${pct}% of $${comp.amount.toLocaleString()} salary = $${cost.toLocaleString()}`);
          break;
        }
        case 'flat_client_fee': {
          const cost = calcFlatFee(comp.amount, assignment.flatFeeOverride);
          totalCost += cost;
          parts.push(`Flat fee $${cost.toLocaleString()}`);
          break;
        }
        case 'hourly': {
          const rate = assignment.hourlyRateOverride ?? comp.amount;
          // Default estimate: 20 hrs/month if no hours tracked
          const cost = calcHourlyEstimate(rate, 20);
          totalCost += cost;
          parts.push(`$${rate}/hr × 20 hrs est. = $${cost.toLocaleString()}`);
          break;
        }
        case 'revenue_share': {
          ...
          totalCost += cost;
          parts.push(`${(pct * 100).toFixed(0)}% rev share = $${cost.toLocaleString()}`);
          break;
        }
        case 'profit_share': {
          const pct = assignment.profitShareOverride ?? comp.sharePercent ?? 0;
          // Profit estimation: revenue - team costs (simplified as 60% of revenue)
          const rev = comp.appliesToCategory ? (revenueMap.get(comp.appliesToCategory) ?? 0) : 0;
          const estProfit = rev * 0.6;
          const cost = calcProfitShare(pct, estProfit, comp.capAmount);
          totalCost += cost;
          parts.push(`${(pct * 100).toFixed(0)}% profit share = $${cost.toLocaleString()}`);
          break;
        }
      }
    }
    ...
    return {
      ...
      compensationBasis: memberComps.map(c =>
        c.componentType === 'salary_allocation' ? 'Salary' :
        c.componentType === 'flat_client_fee' ? 'Flat Fee' :
        c.componentType === 'hourly' ? 'Hourly' :
        c.componentType === 'revenue_share' ? 'Rev Share' : 'Profit Share'
      ).join(' + '),
